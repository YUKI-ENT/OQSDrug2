...
- **AI機能**：`Ollama` または **OpenAI互換API** を提供するAIサーバーが利用できる環境では、xml薬歴取得後、**自動で投薬内容から疾患状況を推論して表示**可能
- 薬剤添付文書も本ツールからインターネット接続無しで表示可能
- 添付文書情報もPostgreSQLに構造化した状態（JSON形式）で保存してありますので、今後LLMの学習等に利用加工が可能
  
となっております。

|   環境      | PostgreSQLなし      | PostgreSQLあり・AIサーバーなし | PostgreSQLあり・AIサーバーあり                 |
|---------------|---------------------|-----------------------|-----------------------------|
...
- **AIサーバー**
  - OQSDrug2は、**Ollama** に加えて **OpenAI互換API** を提供するAIサーバーにも対応しています。AIサーバーが利用できる環境では、**病名・病態等の推論機能**が利用できます。
  - Ollamaを利用する場合、病名の推論では `gemma3:12b`、`gpt-oss:20b` あたりが安定した出力でした。これらのモデルを動かすには、16GB以上のVRAMを積んだNvidia製GPUが目安です。当方では、**CPU Intel Corei9-9900、メモリ32GB、GeForce RTX5080 16GB / Ubuntu24.04** の環境で安定動作を確認しました。
  - OllamaはWindows、Macでも動作します。[ダウンロードサイト](https://ollama.com/download/windows) からインストールできます。
  - AIサーバーを別PCに置く場合は、LAN内のOQSDrug2からAPIへアクセスできるように設定してください。
    
- **ダイナミクス**  
  ダイナミクスが本ソフトの必須環境です。従来のMDB直接参照方式に加えて、**COM連携**に対応しました。  
  COM連携を使う場合は、**診察室PCにOQSDrug2をインストールし、同じPCで起動中のダイナミクスクライアントから直接情報を取得**できます。このため、従来必要だった「OQSDrug取込専用機」は不要です。  
  従来どおり、ダイナミクスが動いていないPCからダイナサーバーの`datadyna.mdb`を参照するMDB方式も利用できます。
- **RSBase**  
...
1. **ダイナミクスとの連携方法を選択します。**

   現在は、診察室PCにOQSDrug2をインストールして、**起動中のダイナミクスクライアントとCOM連携する構成を推奨**します。  
   この構成では、従来のような **「OQSDrug取込専用機」は不要**です。

   - **推奨：COM連携**
     - 診察室PCでダイナミクスクライアントとOQSDrug2を同時に起動します。
     - OQSDrug2はCOM経由で、起動中のダイナミクスクライアントから必要な情報を読み取ります。
     - `設定 → 取込設定 → ダイナミクスの場所` で **COM読み取り**を選択します。
   - **従来方式：MDB直接参照**
     - ダイナミクスが動作していないPCにOQSDrug2を設置し、ダイナサーバーの`datadyna.mdb`を参照する従来方式も引き続き利用できます。
     - 必要に応じて、クライアントMDBを指定する従来方式も利用できます。

   **PostgreSQLはダイナLAN内のどこに置いても構いません。**  
   診察室PC、ダイナサーバー、AIサーバーのいずれにも配置できますが、**小規模構成では診察室PCにOQSDrug2と一緒に入れるのが最も手軽**です。

   推奨構成例：

```mermaid
flowchart LR
    OQS["オンライン資格確認端末<br/>OQS共有フォルダ"]

    subgraph EXAM["診察室PC（推奨構成）"]
        DYNA["ダイナミクスクライアント<br/>Microsoft Access"]
        OQSD["OQSDrug2"]
        PG["PostgreSQL<br/>★最も手軽な配置"]
        OQSD <-->|"COM連携"| DYNA
        OQSD <-->|"DB接続"| PG
    end

    subgraph DYNASRV["ダイナサーバー"]
        MDB["datadyna.mdb"]
        PG2["PostgreSQL<br/>配置可能"]
    end

    subgraph AISRV["AIサーバー"]
        AI["LLM API<br/>Ollama / OpenAI互換API"]
        PG3["PostgreSQL<br/>配置可能"]
    end

    OLD["OQSDrug取込専用機<br/>（従来方式では使用可）"]
    ...
    PG2 -. "PostgreSQLは<br/>ここでも可" .- OQSD
    PG3 -. "PostgreSQLは<br/>ここでも可" .- OQSD
```

   ※ PostgreSQLは上図の3か所すべてに置く必要はなく、**いずれか1か所**です。
...
- **初期設定**
  ...
  - **取込設定**
    ...
    - 取込を行うPCでは、ダイナミクスの読取方式として **COM読み取り** または **mdbから読み取り** を選択します。
      - **COM読み取り**：診察室PCで起動中のダイナミクスクライアントから直接読み取ります。ダイナミクスの場所指定は不要です。
      - **mdbから読み取り**：従来方式です。`datadyna.mdb` またはクライアントMDBを指定します。
    - OQSフォルダ（オンライン資格確認端末のoqsフォルダの共有名）、薬剤情報の取込期間、取込間隔を指定できます。
    - COM連携の詳細は [docs/dynamics-com.md](docs/dynamics-com.md) を参照してください。
...
- **AI推論表示**
  - `(AI)病態背景`は、`Ollama` または **OpenAI互換API** のAIサーバー設定がされている場合に機能します。
